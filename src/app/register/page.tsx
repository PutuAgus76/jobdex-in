...
export default function RegisterPage() {
  return (
    <main className="flex min-h-screen flex-col items-center justify-center bg-[var(--jd-neo-bg)] text-[var(--jd-neo-text)] px-4 py-10 transition-colors duration-200">
      ...
      <Card className="w-full max-w-lg bg-[var(--jd-neo-surface)]">
        <CardHeader className="text-center">
          <CardTitle className="text-xl font-black text-[var(--jd-neo-text)]">Register Anggota</CardTitle>
          <p className="mt-2 text-xs leading-relaxed text-[var(--jd-neo-muted)] font-normal">
            Buat akun anggota baru. Role default otomatis disimpan sebagai Anggota.
          </p>
        </CardHeader>
        <CardContent>
          <RegisterForm />
          <p className="mt-6 text-center text-xs text-[var(--jd-neo-muted)] font-normal">
            Sudah punya akun?{" "}
            <Link href="/login" className="font-bold text-[var(--jd-neo-text)] underline hover:text-[#8fa882]">
              Login
            </Link>
          </p>
        </CardContent>
