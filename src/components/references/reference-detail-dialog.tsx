...
export function ReferenceDetailDialog({
  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center overflow-y-auto bg-slate-950/40 px-4 py-8">
      <div className="w-full max-w-4xl rounded-[8px] bg-white shadow-xl">
        <div className="flex items-start justify-between gap-4 border-b border-slate-200 p-5">
          <div>
            <div className="flex flex-wrap gap-2">
              <DesignTypeBadge type={reference.design_type} />
            </div>
            <h2 className="mt-3 text-2xl font-bold text-slate-950">{reference.title}</h2>
          </div>
        ...
        <div className="grid max-h-[78vh] gap-5 overflow-y-auto p-5 lg:grid-cols-[1.1fr_1fr]">
          <div className="space-y-4">
            {reference.thumbnail_url ? (
              ...
              <div className="flex min-h-64 items-center justify-center rounded-[8px] border border-dashed border-slate-300 bg-slate-50 text-sm text-slate-500">
                Tidak ada thumbnail.
              </div>
            )}
            {reference.drive_url ? (
          ...
          <div className="space-y-4">
            ...
            <section>
              <p className="mb-2 text-sm font-semibold text-slate-950">Color palette</p>
